...
int main()
{
	...
	do {
		...
		iResult = recv(ClientSocket, clientMessage, DEFAULT_BUFLEN, 0); // The recv function is used to read incoming data: https://docs.microsoft.com/en-us/windows/win32/api/winsock/nf-winsock-recv
		clientMessage[iResult] = '\0';

		if (iResult > 0) {
			cout << "Клiент пише: " << clientMessage << "\n";
			//Sleep(PAUSE);
			//cout << "байтов получено: " << iResult << "\n";
			//Sleep(PAUSE);
			
			//////////////////////////////////////////////////////////////////////////////////////////////////////////
			const char* answer1 = "Добрий день!";
			const char* answer2 = "Чудово!";
			const char* answer3 = "14:42!";
			const char* answer4 = "Pink Floyd";
			const char* answer5 = "Пока!";
			string finalanswer;

			// Give some answer back to the sender
			if (strcmp(clientMessage, "Привiт") == 0) {
				cout << "сервер отвечает: " << answer1 << "\n";
				finalanswer = answer1;
			}

			if (strcmp(clientMessage, "Як справи?") == 0) {
				cout << "сервер отвечает: " << answer2 << "\n";
				finalanswer = answer2;
			}

			if (strcmp(clientMessage, "Котра година?") == 0) {
				cout << "сервер отвечает: " << answer3 << "\n";
				finalanswer = answer3;
			}

			if (strcmp(clientMessage, "Який твiй улюблений гурт?") == 0) {
				cout << "сервер отвечает: " << answer4 << "\n";
				finalanswer = answer4;
			}

			if (strcmp(clientMessage, "Пока!") == 0) {
				cout << "сервер отвечает: " << answer5 << "\n";
				finalanswer = answer5;
			}

			const char* serveranswer = finalanswer.c_str();
			iResult = send(ClientSocket, serveranswer, strlen(serveranswer), 0); // The send function sends data on a connected socket: https://docs.microsoft.com/en-us/windows/win32/api/winsock2/nf-winsock2-send
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			cout << "Ответ отправлен клиенту: " << finalanswer << "\n";
			break;
		}
